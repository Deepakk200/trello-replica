  ...
  function ListColumn({ boardId, listId }: { boardId: ID; listId: ID }) {
    ...
    return (
      <div
        ...
        className={`anim-list-enter w-68 shrink-0 max-h-full flex flex-col bg-trello-listBg rounded-xl ${isDragging ? 'opacity-20' : ''}`}
      >
        ...
        <SortableContext items={cardIds} strategy={verticalListSortingStrategy}>
          <div
            className="flex-1 overflow-y-auto px-2 space-y-2 pb-1 min-h-1"
            role="list"
            aria-label="Cards"
          >
            {cardIds.length === 0 && (
